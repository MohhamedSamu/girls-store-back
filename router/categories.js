...
router.get("/", async (req, res) => {
  const snapshot = await db.category.get();
...
router.post("/", async (req, res) => {
  const data = req.body
  let retMsg = await db.category.add(data)
...
router
  ...
  .put(async (req, res) => {
    const id = req.params.id;
    delete req.body.id;
    let retMsg = await db.category.doc(id).update(req.body)
    res.send({msg: "Operation update completed", return: retMsg});
  })
  .delete(async (req, res) => {
    const id = req.params.id;
    let retMsg = await db.category.doc(id).delete()
